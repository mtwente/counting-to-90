...
I am using the *nordatlantisk-ft* data set [@twenteNordatlantiskft2026] I compiled for this project, containing voting records of Folketinget members from Greenland and the Faroe Islands. Since Folketinget does not provide ready-to-use open datasets but only metadata [@berntzenParliamentaryOpenData2019, 9], I had to create the pipeline myself. The dataset is the result of a workflow in R, taking a list of MPs as input and then scraping voting records from Folketinget's Open Data Platform [@folketingetFolketingetsAbneData2026]. Both workflow and dataset are version-controlled using git, including long-term archival on Zenodo, and are accompanied by extensive metadata and documentation to ensure reproducibility [@mahrOneTemplateRule2025a]. [Codebook](https://mtwente.github.io/nordatlantisk-ft/docs/codebook.html) and [source code](https://github.com/mtwente/nordatlantisk-ft) are available via GitHub. 
...
#| label: fig-mandates
#| fig-height: 5
#| fig-cap: Timeline of MPs from Greenland and the Faroe Islands during the timeframe covered by this data set. Substitutions shorter than three weeks are plotted as dots for better visibility.
...
This dominance by two political parties is in contrast to the Faroese situation, where the four biggest parties take turns more often [@billeFaeroskeFolketingsmedlemmer2025]. In the time frame covered by this data set, the Faroe Islands have been represented by one left-wing and one right-wing political party in Folketinget for most of the time, as opposed to the Greenlandic MPs having belonged to two left-wing parties.
